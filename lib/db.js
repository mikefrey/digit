...
function initialize(config, cb) {
  ...
  database = new sqlite3.Database(config.file, function (err) {
...
function saveCommit(commit, cb) {
  var sql  = 'INSERT OR REPLACE INTO commits (hash, timestamp, message, weight) VALUES(?, ?, ?, ?, ?)';
  var args = [commit.hash, commit.timestamp, commit.message, commit.weight];
...
function loadCommits(ids, cb) {
  var sql = "SELECT * FROM commits WHERE hash IN('" + ids.join("', '") + "')";
...
module.exports.saveClock  = saveClock;
module.exports.loadClocks = loadClocks;
